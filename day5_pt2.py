...
class HydroThermalVents:
    ...
    def find_line_coordinates(self, x1, y1, x2, y2): 
        ...
        y_coordinates_iter = list(range(min(y_coordinates), max(y_coordinates)+1))
        ...
        if zero_length_line:
            ...
            # coordinate_iter_list needs to be in correct order for diagonal line to be assigned
            # properly.  If first element == first element in incremented list then list was sorted properly

            if y_coordinates[0] != y_coordinates_iter[0]:
                y_coordinates_iter.reverse()

            if x_coordinates[0]!= x_coordinates_iter[0]:
                x_coordinates_iter.reverse()

            # x and y coordinate list have to be the same length by definition so only looping through x
            drawn_coordinates = [[x_coordinates_iter[i], y_coordinates_iter[i]] for i in range(len(x_coordinates_iter))]


        elif is_vertical:
            drawn_coordinates = [[x1, y_coordinate] for y_coordinate in y_coordinates_iter]
    ...
    def create_board_diagram(self):
        ...
        rows = [self.no_line_indicator]*(max(x_coordinates) + 1)
        # adding 1 to ensure iteration over max y value
        self.board_diagram = [rows[:] for _ in range(max(y_coordinates) + 1)]
